...
router.post('/', wrapAsync(async (req, res) => {
    ...
    await Discussion_boardsService.create(req.body.data, req.currentUser, true, link.host);
    const payload = true;
...
router.put('/:id', wrapAsync(async (req, res) => {
  await Discussion_boardsService.update(req.body.data, req.body.id, req.currentUser);
  const payload = true;
  res.status(200).send(payload);
}));
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await Discussion_boardsDBApi.findBy(
    { id: req.params.id },
  );
