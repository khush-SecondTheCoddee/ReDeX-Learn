...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/discussion_boards/discussion_boardsSlice'
...
const TableSampleDiscussion_boards = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { discussion_boards, loading, count, notify: discussion_boardsNotify, refetch } = useAppSelector((state) => state.discussion_boards)
  ...
  useEffect(() => {
      if (discussion_boardsNotify.showNotification) {
          notify(discussion_boardsNotify.typeNotification, discussion_boardsNotify.textNotification);
      }
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `discussion_boards`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
