...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/instructors/instructorsSlice'
...
const TableSampleInstructors = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { instructors, loading, count, notify: instructorsNotify, refetch } = useAppSelector((state) => state.instructors)
  ...
  useEffect(() => {
      if (instructorsNotify.showNotification) {
          notify(instructorsNotify.typeNotification, instructorsNotify.textNotification);
      }
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `instructors`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
