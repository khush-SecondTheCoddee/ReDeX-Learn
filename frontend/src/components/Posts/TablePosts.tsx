...
import { fetch, update, deleteItem, setRefetch, deleteItemsByIds } from '../../stores/posts/postsSlice'
...
const TableSamplePosts = ({ filterItems, setFilterItems, filters, showGrid }) => {
  ...
  const { posts, loading, count, notify: postsNotify, refetch } = useAppSelector((state) => state.posts)
  ...
  useEffect(() => {
    loadColumns(handleDeleteModalAction, `posts`).then((newCols) =>
      setColumns(newCols),
    );
  }, []);
